...
local toast_timeout_ms = 2000

---------------------------------------------------------------
--- functions
---------------------------------------------------------------
local function get_last_non_empty_input_text(pane, semantic_zones, last_output_zone_index)
	for i = last_output_zone_index - 1, 1, -1 do
		local zone = semantic_zones[i]
		if zone.semantic_type == "Input" then
			local command = utils.trim_trailing_whitespace(pane:get_text_from_semantic_zone(zone))
			if command ~= "" then
				return command
			end
			return nil
		end
		if zone.semantic_type == "Prompt" or zone.semantic_type == "Output" then
			return nil
		end
	end
	return nil
...
local function get_last_command_text(pane, semantic_zones, last_output_zone_index)
	local command = get_last_non_empty_input_text(pane, semantic_zones, last_output_zone_index)
	if command ~= nil then
		return command
	end

	local current_command = utils.trim_trailing_whitespace((pane:get_user_vars() or {}).WEZTERM_PROG or "")
	if current_command ~= "" then
		return current_command
	end

	return utils.get_last_executed_command(pane:pane_id())
end

local function copy_last_command_output(window, pane)
	if pane:is_alt_screen_active() then
		utils.show_transient_left_status(window, "Cannot copy shell output from alternate screen", toast_timeout_ms)
		return
	end

	local semantic_zones = pane:get_semantic_zones()
	local last_output_zone_index = nil
	for i = #semantic_zones, 1, -1 do
		if semantic_zones[i].semantic_type == "Output" then
			last_output_zone_index = i
			break
		end
	end

	if last_output_zone_index == nil then
		utils.show_transient_left_status(window, "No shell output zone found", toast_timeout_ms)
		return
	end

	local last_output_zone = semantic_zones[last_output_zone_index]
	local output = pane:get_text_from_semantic_zone(last_output_zone)
	if output == "" then
		utils.show_transient_left_status(window, "Last command produced no output", toast_timeout_ms)
		return
	end

	local command = get_last_command_text(pane, semantic_zones, last_output_zone_index)

	local text = output
	if command ~= nil and command ~= "" then
		local separator = string.rep("=", math.max(#command, 3))
		text = string.format("%s\n%s\n%s\n%s", separator, command, separator, output)
	end

	window:copy_to_clipboard(text, "ClipboardAndPrimarySelection")
	utils.show_transient_left_status(window, "Copied!", toast_timeout_ms)
end
...
M.default_keybinds = {
	...
	{ key = "c", mods = "ALT|SHIFT", action = wezterm.action_callback(copy_last_command_output) },
}
